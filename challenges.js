...
function sayHello() {
  ...
  function computeRemainder (dividend, divisor) {
    ...
    //return dividend % divisor; //BORING WAY
    //check that adding divisor to cummulative wont be greater than dividend
    let cummulative = 0;
    while (cummulative + divisor <= dividend) {
      //if it wont be, add divisor to cumulative and check again
      cummulative += divisor;
    }
    //if it will be, return dividend - cummulative
    return dividend - cummulative;
  }
  ...
  function reverseUpcaseString (string) {
    ...
    let revUpString = revString.toUpperCase();
    return revUpString
  }
  ...
  function removeEnds(string) {
    tempString = string.slice(0, -1);
    tempString = tempString.slice(1, string.length);
    return tempString;
  }
  ...
  //I like my code better than solution ex, but Jasmine thinks it isn't counting correctly...
  ...
  function formatWithPadding(cont, peanut, min) {
    //turn split content and turn into an array 
    cont = cont.toString();
    array = cont.split('');
    //if content is longer than minimum, return array turned into string
    if (array.length >= min) {
      return array.join('');
    } else {
      let padVal = min - array.length;
      for (i=0; i < padVal; i++) {
        array.unshift(peanut);
      }
      return array.join('');
    }
  }
  ...
  function isPalindrome(string) {
    ...
    let compareStrOne = cleanString.split(' ').join('');
    let cleanArray = compareStrOne.split('');
    let revArray = cleanArray.reverse();
    let compareStrTwo = revArray.join('');
    if (compareStrOne === compareStrTwo) {
      return true;
    } else {
      return false;
    }
  ...
  function hammingDistance(str1, str2) {
    //if the strings are not the same length, return NaN
    if (str1.length !== str2.length) return NaN;
    //calculate number of differences at same position
    let numdif = 0;
    let arr1 = str1.split('');
    let arr2 = str2.split('');
  ...
  function findHighestPriced(arr) {
    ...
    arr.forEach(function(o) {
      if (o.price > currHighPrice) {
        currHighPrice = o.price
        highestIdx = idxCounter;
      }
    idxCounter += 1;
